...
def degree_heuristic(csp: CSP) -> List[str]:
    degree = {var: 0 for var in csp.domains}

    for constraint in csp.constraints:
        for var in constraint.scope:
            degree[var] = degree.get(var, 0) + len(constraint.scope) - 1

    return sorted(csp.domains.keys(), key=lambda var: degree[var], reverse=True)
